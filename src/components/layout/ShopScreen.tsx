      ...
      tg.openInvoice(data.invoiceUrl, (status: string) => {
        if (status === 'paid') {
          if (data.type === 'premium') {
            // Premium purchased — update state
          ...
          {PREMIUM_PLANS.map((plan, i) => (
            <motion.div
              ...
              className={`rounded-2xl p-3.5 border relative overflow-hidden ${
                plan.popular
                  ? 'bg-gradient-to-r from-mystic-gold/15 to-mystic-accent/10 border-mystic-gold/40'
                  : 'bg-mystic-card/80 border-mystic-gold/15'
              }`}
            >
              {plan.popular && (
                <div className="absolute top-0 right-0 bg-gradient-to-l from-mystic-gold to-mystic-accent text-mystic-bg text-[9px] font-bold px-2.5 py-0.5 rounded-bl-xl">
                  {T.popular[l]}
                </div>
              )}
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-xl font-bold text-sm transition-all ${
                    ...
                    plan.popular
                      ? 'bg-gradient-to-r from-mystic-gold to-mystic-accent text-mystic-bg'
                      : 'bg-mystic-gold/20 border border-mystic-gold/30 text-mystic-gold'
                  }`}
                >
