...
Последняя фраза — не общий вывод, а одно конкретное действие: что сделать или сказать в первую очередь, исходя из этого ответа.`;
}
...
Твой ответ: Структурируй ровно в этом порядке, каждая карта — отдельная строка в формате **Позиция** — **Карта**: текст. Каждую карту опиши одним-двумя предложениями, как таролог, который видит не просто картинку, а энергию, текущую сквозь время.
...
В строке про Будущее — не просто прогноз, а одно конкретное действие уже на этой неделе, которое реально влияет на то, куда течёт линия.`;
}
...
Каждую карту раскрой в одно-два предложения, каждая — отдельная строка в формате **Позиция** — **Карта**: текст. Язык — мистический, но не перегруженный. Создай ощущение, что ты действительно вскрыл сокрытое.

Заверши не общей фразой об отношениях, а конкретным шагом: что сделать или сказать в ближайшие дни, исходя из того, что сейчас у него на уме.`;
}
...
Каждую позицию объясни одним-двумя предложениями, каждая — отдельная строка в формате **Позиция** — **Карта**: текст. Включи практическую мудрость: не просто "Император" — а что это означает для твоей карьеры прямо сейчас.

Заверши не общей фразой о тренде, а одним конкретным шагом на этой неделе, который реально сдвигает ситуацию с деньгами или карьерой в лучшую сторону.`;
}
...
Ответь в формате (каждый день — отдельная строка):
**Понедельник** — **Название карты**: [1-2 фразы]
**Вторник** — **Название карты**: [1-2 фразы]
...и так далее.
...
Заверши не общей фразой о линии недели, а одним конкретным действием, с которого стоит начать в понедельник, чтобы неделя прошла по лучшему из возможных сценариев.`;
}
...
Отвечай как историк времени. Каждую неделю опиши одним-двумя предложениями в формате **Неделя N** — **Карта**: текст, ловя сюжет, который развивает таро.

Покажи дугу месяца: восход, пик, спуск. Что нужно пережить? Что получится в конце?

Заверши не общим прогнозом, а одним конкретным шагом на первую неделю месяца — тем, что задаёт тон всей дуге.`;
}
...
3) Один конкретный шаг — что сделать в первую очередь, а не общая мысль вроде "доверься себе"
...
ВОСХОЖДЕНИЕ (твоё путешествие и итог):
...
2. ЧЕТЫРЕ СТОРОНЫ (1-2 фразы на каждую, формат **Позиция** — **Карта**: текст): Раскрой каждую сторону. Не просто значения карт — покажи, как они взаимодействуют, как одна усиливает другую.

3. ВОСХОЖДЕНИЕ (1-2 фразы на каждую, формат **Позиция** — **Карта**: текст): Это подъём. Как ты видишь себя? Что говорят люди? Чего ты боишься или ждёшь?

4. ФИНАЛ (3-4 фразы): Посмотри на карту 10 — это ответ Таро. Но не просто прочти её значение. Покажи, КАК она связана с первыми картами. Это завершение истории. Заверши не общим напутствием, а одним конкретным шагом на ближайшие дни, который прямо вытекает из карты 10.
