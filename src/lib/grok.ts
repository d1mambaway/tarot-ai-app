...
Пиши на ${lang} языке. СТРОГО на ${lang} — ни единого слова на английском, испанском, немецком или любом другом языке. Допустимы только: MC, ASC, IC, DC.

🔮 МАСТЕР-ПРОМТ ДЛЯ ГЕНЕРАЦИИ НАТАЛЬНОЙ КАРТЫ

⚠️ КРИТИЧЕСКИЕ ПРАВИЛА (НЕ НАРУШАТЬ)

1. БЕЗ ПОВТОРЕНИЙ
• Каждая фраза может появиться ТОЛЬКО ДВА РАЗА во всём документе максимум
• Запрещено повторять:
  ◦ "Ты — человек, который..."
  ◦ "Ты имеешь способность..."
  ◦ "Практикуй..."
  ◦ "Если ты не научишься..."
• Проверь документ перед выводом на повторения. Если нашёл — переписать фразу
• Каждая секция должна быть максимально уникальна

2. ТОЛЬКО "ТЫ" (личное обращение)
• Все "Вы" заменить на "Ты"
• Это делает текст теплее и персональнее
• Тон: как совет астролога другу, а не учебник

3. СТРУКТУРА КАЖДОЙ ПЛАНЕТЫ/АСПЕКТА
Обязательно три блока:

[НАЗВАНИЕ ПЛАНЕТЫ/АСПЕКТА]
...
⚠️ Слепая зона:
- Какой риск или ограничение скрывается в этой энергии
- Что может пойти не так, если не работать с этим

✨ Действие:
- КОНКРЕТНЫЙ, практический совет
- НЕ просто "медитируй" — скажи ЧТО медитировать
- Действие должно быть уникальным для этой планеты
- Не копировать действия из других секций

4. БЕЗ ШАБЛОННЫХ ФРАЗ
Запретные начала (забудь о них):
❌ "Ты должен/должна..."
❌ "Ты должен научиться..."
❌ "Указывает на то, что..."
...
❌ "Старайся..."
Правильные подходы:
✅ "Твоя энергия естественным образом..."
✅ "Это проявляется в том, что..."
✅ "Риск здесь в том, что..."
✅ "Практикуй [конкретное действие], чтобы [конкретный результат]"

5. МАТЕМАТИКА ДОЛЖНА БЫТЬ ПРАВИЛЬНОЙ
• Если считаешь стихии: Сумма всегда = 10
• Если говоришь о доминировании: проверь цифры
• Пример: Огонь 3, Воздух 3 — НЕ "сильное доминирование", а "сбалансированный баланс"

6. АСТРОЛОГИЧЕСКАЯ ТОЧНОСТЬ
• Сатурн в Рыбах = сомнения, страхи, внутренние ограничения, а НЕ дисциплина
• Марс ретроград = энергия направлена внутрь, но это не слабость, а другой способ действия
• Северный Узел = куда расти, Южный Узел = от чего отпускать
• Лилит = скрытая сила и тень, не осуждай, опиши как инструмент

7. БЕЗ ОПЕЧАТОК
• Проверь перед выводом на:
  ◦ Смешанную кириллицу и латиницу ("открытым", не "открытým")
  ◦ Потерянные части слов ("иться", "умённый")
  ◦ Пропущенные глаголы ("призван тайны" → неправильно)
• Если нашёл ошибку — переписать предложение целиком

8. СТИХИИ И ЭНЕРГИЯ

Огонь: [цифра] | Земля: [цифра] | Воздух: [цифра] | Вода: [цифра]

[Описание: что это означает ДЛЯ ЭТОГО ЧЕЛОВЕКА конкретно]

⚠️ Дисбаланс (если есть):
[Как компенсировать недостаток или избыток]

✨ Совет:
[Практическое применение для баланса]
...
🌟 ЯДРО ЛИЧНОСТИ — БОЛЬШАЯ ТРОЙКА
  - Солнце [описание → слепая зона → действие]
  - Луна [описание → слепая зона → действие]
  - Асцендент [описание → слепая зона → действие]
  - Синтез (как работают вместе)

🪐 ЛИЧНЫЕ ПЛАНЕТЫ — КАК ТЫ ДЕЙСТВУЕШЬ В МИРЕ
  - Меркурий [описание → слепая зона → действие]
  - Венера [описание → слепая зона → действие]
  - Марс [описание → слепая зона → действие]
  - Юпитер [описание → слепая зона → действие]
  - Сатурн [описание → слепая зона → действие]

⚡ КЛЮЧЕВЫЕ АСПЕКТЫ
  - [Каждый аспект: описание → как использовать]

🔥 ЭНЕРГЕТИЧЕСКИЙ БАЛАНС (СТИХИИ)
  - Таблица стихий
  - Описание баланса
  - Как компенсировать дисбалансы

🌙 КАРМИЧЕСКИЙ ВЕКТОР
  - Северный Узел [описание → действие]
  - Южный Узел [описание → действие]

🌑 СКРЫТАЯ СИЛА (ЛИЛИТ)
  - Описание как ресурса, не как проклятия
  - Слепая зона
  - Действие

💫 РЕТРОГРАДЫ
  - [Каждый ретроград: как это проявляется, не как "баг"]

🏠 ПРИЗВАНИЕ И КАРЬЕРА
  - На основе MC и связанных планет
  - Конкретные сферы деятельности
  - Как использовать свою энергию

💝 ОТНОШЕНИЯ И ПАРТНЁРСТВО
  - На основе 7-го дома и Венеры
  - Паттерны в отношениях
  - Как улучшить связь

🎯 КАРТА ДЕЙСТВИЙ
  - Три сильные стороны (конкретные)
  - Три зоны роста (конкретные)
  - Главный совет (уникальный для карты)

✨ ПОРТРЕТ
  - Объединение всех элементов в один образ
  - Уникальное описание этого человека
  - Его миссия на основе карты
...
Объём:
• Каждая планета: 200-300 слов (не менее)
• Каждый аспект: 150-200 слов
• Каждая секция в конце: 200-250 слов
• Всего документа: 2500-3500 слов (достаточно, чтобы было мясо)

Тон:
• Персональный (как астролог говорит с клиентом)
• Не патерналистский ("ты должен") — описательный
• Баланс между позитивом и реалистичностью
• Каждый совет практичен и реализуем

Глубина:
• Не поверхностные описания
• Связывать планеты с домами (например, Марс в 5-м доме — не то же самое, что Марс в 1-м)
• Показывать конфликты и синергию между элементами
• Объяснять КАК это проявляется в повседневной жизни

═══════════════════════════════════════════════════════════════
ПРИМЕРЫ ПРАВИЛЬНЫХ/НЕПРАВИЛЬНЫХ ФРАЗ
═══════════════════════════════════════════════════════════════

❌ НЕПРАВИЛЬНО:
"Ты должна научиться доверять своей интуиции и найти новые пути для роста и развития."

✅ ПРАВИЛЬНО:
"Твоя интуиция работает лучше всего, когда ты замедляешься и слушаешь тишину.
Риск: если ты будешь игнорировать эти внутренние подсказки в пользу логики,
ты можешь принять решения, которые не будут резонировать с твоей истинной природой.
Действие: практикуй день без планов один раз в неделю и записывай, что приходит тебе в голову."

═══════════════════════════════════════════════════════════════
ФИНАЛЬНАЯ ПРОВЕРКА
═══════════════════════════════════════════════════════════════

Перед выводом документа проверь:
□ Нет повторений одной фразы более 2 раз
□ Каждое "действие" уникально и практично
□ Нет шаблонных начал ("ты должна", "указывает на то, что")
□ Стихии суммируются в 10
□ Нет опечаток и смешанной кириллицы
□ Объём достаточный (не менее 2500 слов)
□ Каждая планета связана с домом
□ Аспекты объясняют внутренние конфликты или синергию
□ Ретрограды описаны как особенности, не как недостатки
□ Карьера и отношения основаны на конкретных домах и планетах
□ Портрет в конце уникален для этого человека, а не генеричен

═══════════════════════════════════════════════════════════════
🔴 КРАСНЫЕ ФЛАГИ (что делать НЕЛЬЗЯ)
═══════════════════════════════════════════════════════════════

1. Если одна фраза повторяется более 1 раза → ПЕРЕПИСАТЬ
2. Если действие копирует действие из другой секции → ПРИДУМАТЬ НОВОЕ
3. Если описание планеты звучит как копия другой → РАЗВЕРНУТЬ КОНКРЕТНЕЕ
4. Если нет конкретного примера "как это проявляется в жизни" → ДОБАВИТЬ
5. Если "риск" или "действие" можно применить к 3+ планетам → ПЕРЕДЕЛАТЬ

ПРИМЕРЫ ТОГО, ЧТО ИСКАТЬ И ИСПРАВЛЯТЬ:
❌ "ты естественным образом притягиваешься к..." — шаблон, повторяется
❌ "практикуй медитацию" — слишком общее действие
❌ "может стать слишком разнообразным" — применимо к чему угодно
❌ "доверяй своей интуиции" — если уже написано в другой секции
❌ "развивай свою уникальность" — пустая фраза без конкретики

✅ "В конфликтах ты первым замолкаешь, а потом неделю прокручиваешь диалог в голове"
✅ "Попробуй записывать три вещи, которые тебя вдохновили сегодня — через месяц увидишь паттерн"
✅ "Твоя склонность прыгать между идеями может быть преобразована в сверхбыструю адаптацию"
✅ "Каждое утро выбирай ОДНУ задачу и не открывай вторую, пока не закончишь первую"
✅ "Заведи папку «безумные идеи» и раз в месяц выбирай оттуда одну для реализации"

ПРОЦЕДУРА САМОПРОВЕРКИ:
После написания каждой секции — перечитай ВСЕ предыдущие секции и убедись:
• Ни один совет не дублируется
• Ни одна метафора не повторяется
• Каждый "риск" специфичен именно для этой планеты/аспекта
• Каждое "действие" невозможно перепутать с действием из другой секции
Если нашёл совпадение — ПЕРЕПИШИ новую секцию, а не старую.

💡 ЗОЛОТОЕ ПРАВИЛО
Каждый раз, когда хочешь написать шаблонную фразу — ОСТАНОВИ СЕБЯ и переформулируй.
Если у тебя получается текст, который я буду читать и думать "вау, это конкретно, интересно, применимо" — значит ты сделал правильно.
Если у меня есть ощущение "это AI скопировал сам себя" — значит нарушил правила.

ГОТОВО? Генерируй натальную карту по этому промту.`;
}
